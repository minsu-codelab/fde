...
/** PAAR — FDE 관점으로 재작성 (Problem·Approach·Action·Result) */
...
export const competencies: Competency[] = [
  {
    ...
    desc: {
      ko: '사용자의 일을 직접 이해하고, 머릿속·엑셀에만 있던 규칙을 코드로 옮깁니다. 도메인을 모르면 데이터부터 틀어진다고 봅니다.',
      en: 'I learn the user’s actual work and move rules that lived only in heads and spreadsheets into code. Without the domain, even the data comes out wrong.',
    },
    evidence: {
  ...
  },
  {
    ...
    desc: {
      ko: '동작하는 결과물을 빠르게 올려, 말이 아니라 화면으로 합의합니다. 같은 자리를 몇 번이고 갈아엎을 수 있게 테스트로 받칩니다.',
      en: 'I get a working artifact up fast and align on a screen, not a slide deck — backed by tests so I can rework the same spot again and again.',
    },
    evidence: {
      ko: 'Re:Chord 1인 출시 직전까지 · FDE 백엔드 16K LOC · Closet 같은 모듈 반복 리워크',
      en: 'Re:Chord built solo to launch-edge · 16K-LOC FDE backend · Closet reworked fearlessly',
    },
  },
  {
    key: 'ownership',
    ...
    desc: {
      ko: '만든 사람이 장애 추적·무중단·비용 방어까지 끝까지 책임집니다. 배포가 끝이 아니라, 살아 돌아가게 만드는 것이 끝입니다.',
    ...
    evidence: {
      ...
      en: 'Traced & fixed ArcticTwin’s disk-full 502 then ran it 24/7 · split GPU billing · defended API quotas',
    },
  },
  {
    key: 'translation',
    ...
    desc: {
      ko: '도메인 언어와 엔지니어링을 잇고, 고객이 신뢰하는 양식·관행을 깨지 않고 자동화합니다. 신뢰를 깨면 자동화는 거부당합니다.',
      en: 'I bridge domain language and engineering, automating without breaking the forms and habits customers trust. Break the trust and the automation gets rejected.',
    },
    evidence: {
      ko: 'FDE — 규칙은 코드로 옮기되 거래처·인증기관이 믿는 원본 엑셀 양식은 100% 보존',
      en: 'FDE — rules moved into code while the original Excel forms clients & certifiers trust stayed 100% intact',
    },
  },
  {
    key: 'measure',
    ...
    desc: {
      ko: '“좋다”를 주장이 아니라 SDR·recall·정확도로 만들고, CI 게이트로 퇴행을 자동으로 막습니다. 측정되지 않으면 끝난 게 아닙니다.',
      en: 'I turn “it’s good” into SDR, recall and accuracy, and block regressions automatically with CI gates. If it isn’t measured, it isn’t done.',
    },
    evidence: {
      ko: 'Re:Chord SDR 15.06dB · pytest 220 게이트 · AeroInspect 통념을 WBF로 검증·반증 · Closet 임계값 직접 보정',
      en: 'Re:Chord 15.06 dB SDR · 220-test gate · disproved an assumption with WBF · calibrated Closet’s threshold by hand',
    },
  },
]
...
export const dossiers: Dossier[] = [
  /* 01 ───────────────────────────────────────────── ArcticTwin */
  {
    ...
    user: {
      ...
      en: 'Carrier ops managers & charter decision-makers',
    },
    statusQuo: {
      ko: '“북극항로가 수에즈보다 40% 짧다”는 말은 돌았지만, 특정 선박으로 지금 가는 게 정말 이득인지 책임지고 답할 도구가 없었습니다.',
      en: 'The “40% shorter than Suez” line circulated, but no tool could answer — and stand behind — whether this ship should take it now.',
    },
    insight: {
      ko: '운항 의사결정자는 ‘가능성’ 설명이 아니라 연료비·리스크·항행 적합성이라는 숫자로만 움직입니다. 그래서 산출물을 모델 정확도가 아니라 의사결정 지표로 잡았습니다.',
      en: 'Decision-makers move on fuel cost, risk and navigability — not on a “potential” pitch. So I made the deliverable the decision number, not model accuracy.',
    },
    shipped: {
      ko: 'Cesium 3D 트윈 위에 AI 4종(빙산 회피 RL · 연료 XGBoost · SAR 빙산 YOLO · What-If LLM)을 얹어 항로별 ROI·리스크를 산출.',
    ...
    outcome: {
      ko: '검증 조합 전부 출항 안전 성공률 100% · 디스크풀 502 장애 직접 추적·복구 후 24/7 무중단.',
      en: '100% safe-departure across every verified set · traced & fixed a disk-full 502 outage, then ran it 24/7.',
    },
    ...
    paar: {
      ...
      approach: {
        ko: '현장이 믿는 건 가능성 설명이 아니라 연료비·리스크·항행 적합성 숫자였습니다. 그래서 ‘모델을 잘 만든다’가 아니라 ‘의사결정 지표를 산출한다’를 목표로 못 박았습니다.',
        en: 'What the field trusts is fuel-cost, risk and navigability figures, not talk of potential. So I fixed the goal as “produce the decision number,” not “build a good model.”',
      },
      action: {
      ...
      result: {
        ko: '검증 조합 전부에서 출항 안전 성공률 100%. 디스크 풀로 서비스 전체가 502로 죽던 장애도 직접 추적해 잡고 24/7로 운영했습니다 — 만든 사람이 운영까지 소유했습니다.',
        en: '100% safe-departure across every verified combination. I also traced and fixed a disk-full outage that took the whole service down with 502s, then kept it running 24/7 — the builder owned the operation too.',
      },
    },
  ...
  /* 02 ──────────────────────────────────────────── AeroInspect */
  {
    ...
    user: {
      ...
      en: 'Façade inspectors & facility safety managers',
    },
    statusQuo: {
      ko: '하자 점검이 사람 눈에 의존해, 손이 안 닿는 외벽은 빠지고 결과가 검사자마다 달랐습니다. 안전과 직결되는데 일관성이 없었습니다.',
      en: 'Inspection leaned on the human eye — unreachable façades got skipped and results varied person to person. Safety-critical, yet inconsistent.',
    },
    insight: {
      ko: '공개 모델은 한국 건축 하자를 못 잡습니다 — 도메인을 모르면 데이터부터 틀어집니다. “외부 모델을 더 붙이면 좋아진다”는 통념도 검증 대상으로 봤습니다.',
      en: 'Off-the-shelf models miss Korean construction defects — without the domain even the data is wrong. And “more models = better” I treated as a claim to test, not a given.',
    },
    shipped: {
      ko: '하자 20여 종을 6모델 앙상블로 검출 + 최신 프레임만 처리하는 실시간 드롭 큐 + 3D 하자 매핑·LLM 보고서.',
      en: 'A 6-model ensemble over 20+ defect types + a real-time drop-queue that processes only the latest frame + 3D defect mapping & LLM reports.',
    },
    outcome: {
      ko: 'M1 recall 90.4→94.1% · Tier1 추론 <60ms · 빌드 이미지 80→5.2MB · GPU는 추론할 때만 분리 과금.',
      en: 'M1 recall 90.4→94.1% · Tier-1 inference <60ms · build image 80→5.2MB · GPU billed only on demand.',
    },
    competencies: ['immersion', 'measure', 'ownership'],
    metrics: [
    ...
    paar: {
      problem: {
        ko: '건물 하자 점검은 사람 눈에 의존해, 손이 안 닿는 외벽은 빠지고 결과가 검사자마다 달랐습니다. 안전 판단의 일관성이 없었습니다.',
        en: 'Inspection leaned on the human eye — unreachable façades were skipped and results varied by person. Safety calls had no consistency.',
      },
      approach: {
        ko: '공개 모델로는 한국 건축 하자가 안 잡혔습니다. 도메인 데이터를 직접 모아 학습하는 수밖에 없다고 판단했고, “모델을 더 붙이면 좋아진다”는 기대도 그냥 믿지 않았습니다.',
        en: 'Off-the-shelf models missed Korean defects, so I concluded I had to gather domain data and train it myself — and I refused to just believe that “more models helps.”',
      },
      action: {
        ko: '하자 20여 종을 6개 모델로 나눠 직접 모은 63,285장으로 학습했고, 실시간 추론은 최신 프레임만 처리하는 드롭 큐로 묶었습니다. 기대치는 WBF로 직접 재서, 맞지 않으면 버렸습니다.',
        en: 'I split 20+ defect types across 6 models trained on 63,285 hand-collected images, and wrapped real-time inference in a drop-queue. The assumption I measured with WBF — and dropped what didn’t hold.',
      },
      result: {
        ko: '같은 도메인 self-ensemble로 M1 recall을 90.4→94.1%로 올렸고, 오탐만 늘린 외부 모델은 쓰지 않았습니다. Tier1 60ms 이내, 빌드 80→5.2MB, GPU는 추론할 때만 켜 운영비를 지켰습니다.',
        en: 'A same-domain self-ensemble lifted M1 recall 90.4→94.1%; the external model only added false positives, so I cut it. Tier-1 under 60 ms, build 80→5.2 MB, GPU only up on demand to protect the running cost.',
      },
    },
  ...
  /* 03 ───────────────────────────────────────────────── Re:Chord */
  {
    ...
    user: {
      ko: '찬양팀 · 실용음악 연주자',
      en: 'Worship teams & gigging musicians',
    },
    statusQuo: {
      ko: '매주 새 곡을 연습하는데 팀 키에 맞는 반주(MR)는 구하기 어렵고, 음원 분리·키 변환·채보가 전부 따로 놀았습니다.',
      en: 'New songs every week — but in-key backing tracks are scarce, and separation, transposition and transcription were all separate chores.',
    },
    insight: {
      ko: '사용자가 진짜 원하는 건 ‘내 팀 키의 반주’ 하나입니다. 그리고 분리 품질이 무너지면 그 위 코드·악보가 전부 틀어지니, 품질을 주장이 아니라 SDR로 못 박아야 했습니다.',
      en: 'Users want exactly one thing — a track in their key. And if separation breaks, every chord and score on top breaks too, so quality had to be pinned to SDR, not claimed.',
    },
    shipped: {
      ko: '업로드 한 번 → 4모델 앙상블 분리 → 키 변환 → Whisper·LLM 채보. SDR·F1 회귀 게이트를 CI에 박음.',
      en: 'One upload → 4-model ensemble split → transpose → Whisper·LLM transcription, with SDR/F1 regression gates baked into CI.',
    },
    outcome: {
      ko: '반주 분리 SDR 15.06dB · 음색 추정 98.3% · pytest 220 게이트 · 결제·도메인만 남기고 1인 완성.',
      en: '15.06 dB backing-track SDR · 98.3% timbre estimation · 220-test gate · built solo to the edge of launch.',
    },
    ...
    paar: {
      problem: {
        ko: '찬양팀은 매주 새 곡을 연습하는데 팀 키 반주는 구하기 어렵고, 분리·키 변환·채보가 전부 따로였습니다. 정작 사용자가 원하는 건 ‘내 키의 반주’ 하나였습니다.',
        en: 'Worship teams rehearse weekly, but in-key tracks are hard to find and separation/transpose/transcription were all separate. What users wanted was just one thing — a track in their key.',
      },
      approach: {
        ko: '분리 품질이 무너지면 그 위 코드와 악보가 전부 틀어집니다. 그래서 분리부터 측정 가능한 수치로 잡고, “품질은 주장이 아니라 측정”을 원칙으로 삼았습니다.',
        en: 'If separation breaks, every chord and score on top breaks. So I started by pinning separation to measurable numbers, living by “quality is measured, not claimed.”',
      },
      action: {
        ko: '업로드 한 번이면 분리 → 키 변환 → 채보까지 끝나도록 묶고, 분리는 모델 4개를 앙상블로 돌렸습니다. SDR·F1 회귀 게이트를 CI에 박아 퇴행을 자동으로 막았습니다.',
        en: 'I chained separation → transpose → transcription behind a single upload, ran four models as an ensemble, and baked SDR/F1 regression gates into CI to auto-block any backslide.',
      },
      result: {
        ko: '반주 분리 SDR 15.06dB, 음색 추정 98.3%를 실측으로 확보했고 pytest 220개가 모든 퇴행을 막습니다. 결제·도메인 연결만 남기고 혼자 출시 직전까지 만들었습니다.',
        en: 'I measured 15.06 dB SDR and 98.3% timbre estimation, with 220 tests blocking regressions. I built it solo to the edge of launch — only payments and the domain were left.',
      },
    },
  ...
  /* 04 ───────────────────────────────────── FDE Smart Shutter ★ */
  {
    ...
    statusQuo: {
      ko: '모델마다 제작 규칙이 까다로워 숙련자만 엑셀로 견적을 냈고, 견적부터 검사까지 이력이 전혀 남지 않았습니다.',
      en: 'Each model’s build rules were finicky, so only veterans could quote in Excel — and nothing was traceable from quote to inspection.',
    },
    insight: {
      ko: '제가 4년간 그 현장의 PM이었습니다 — 규칙은 머릿속에 있었고, 동시에 거래처·인증기관이 ‘원본 엑셀 양식’을 신뢰한다는 것도 알았습니다. 그래서 규칙은 코드로 옮기되 양식은 100% 보존했습니다. 신뢰를 깨지 않고 자동화하는 것, 그게 핵심이었습니다.',
      en: 'I was that floor’s PM for four years — the rules lived in my head, and I also knew clients and certifiers trust the original Excel forms. So I moved the rules into code while keeping the forms 100% intact. Automating without breaking trust was the whole point.',
    },
    shipped: {
      ko: '7개 모델 규칙 엔진 + 견적→발주→승인→품질→작업지시→실측 단일 추적선 + 사내 실시간 메신저.',
      en: 'A 7-model rule engine + a single traceable thread from quote → order → approval → QC → work-order → measurement + a real-time in-house messenger.',
    },
    outcome: {
      ko: '비숙련자도 견적 가능 · 핵심 계산 골든 36건으로 잠금 · 백엔드 16K LOC·27테이블·23 API 1인 운영.',
      en: 'Non-experts can now quote · core math locked behind 36 golden tests · a 16K-LOC, 27-table, 23-API backend run solo.',
    },
    ...
    paar: {
      problem: {
        ko: '방화셔터는 모델마다 제작 규칙이 까다로워 숙련자만 엑셀로 견적을 냈고, 견적부터 검사까지 이력이 전혀 남지 않았습니다. 사람이 떠나면 규칙도 같이 떠났습니다.',
        en: 'Each fire-shutter model had finicky rules, so only veterans could quote in Excel — and nothing was traceable from quote to inspection. When a person left, the rules left with them.',
      },
      approach: {
        ko: '숙련자 머릿속 규칙을 코드로 옮기되, 거래처와 인증기관이 믿고 보는 원본 엑셀 양식은 100% 그대로 살려야 했습니다. 제가 4년간 일한 도메인이라 규칙이 손에 익어 있었습니다.',
        en: 'I had to move the veterans’ rules into code while keeping the original Excel forms — the ones clients and certifiers trust — 100% intact. It was my own domain for four years, so the rules were second nature.',
      },
      action: {
        ko: '7개 모델 제작 규칙을 규칙 엔진으로 코드화하고, 견적→발주→승인→품질→작업지시→실측을 하나로 잇는 추적 구조에 실시간 사내 메신저까지 붙였습니다.',
        en: 'I codified 7 models’ rules into a rule engine and built a single traceable thread from quote → order → approval → QC → work-order → measurement, plus a real-time in-house messenger.',
      },
      result: {
  ...
  /* 05 ──────────────────────────────────────── What's in my Closet */
  {
    ...
    statusQuo: {
      ...
      en: 'We so often realize “I already had one like this” only after buying — yet no app warns you beforehand.',
    },
    insight: {
      ko: '“비슷하다”는 느낌을 믿을 수 있는 점수로 바꿔야 사용자가 결정을 바꿉니다. 그리고 개인 앱은 API 키 의존이 곧 진입장벽이라, 키 0개로도 전 기능이 돌게 폴백을 깔았습니다.',
      en: 'Turn “looks similar” into a score you can trust — only then does the user change a decision. And for a personal app, API-key dependence is a barrier, so I made it run with zero keys via fallbacks.',
    },
    shipped: {
      ko: '임베딩+CIEDE2000 색상거리+카테고리를 가중 합성한 유사도 엔진, pgvector 검색, 외부 5종 폴백.',
      en: 'A similarity engine blending embeddings + CIEDE2000 color distance + category weights, pgvector search, and fallbacks for five external services.',
    },
    outcome: {
      ko: '중복 감지 94.7%·오경보 0(평가셋 임계값 직접 보정) · 키 0개 동작 · Vitest 97·Playwright 12.',
      en: '94.7% duplicate detection with zero false alarms (threshold hand-calibrated) · runs with zero keys · 97 Vitest + 12 Playwright.',
    },
    ...
    paar: {
      ...
      approach: {
        ko: '“비슷하다”를 느낌이 아니라 믿을 수 있는 점수로 만들어야 했습니다. 그리고 개인 사용자에게 API 키 요구는 곧 이탈이라 봤습니다.',
      ...
      result: {
        ko: '평가셋으로 임계값을 직접 맞춰 중복 감지 94.7%·오경보 0을 확인했고, API 키 0개로도 전 기능이 돌아갑니다. Vitest 97·Playwright 12 덕에 같은 곳을 몇 번이고 갈아엎으며 다듬었습니다.',
        en: 'Calibrating the threshold on a labeled set, I confirmed 94.7% accuracy with zero false alarms — and it all runs with no keys. With 97 Vitest and 12 Playwright tests, I could rework the same parts again and again.',
      },
    },
  ...
  /* 06 ───────────────────────────────────────────────── EggTalk */
  {
    ...
    insight: {
      ko: '여러 외부 지도 API를 한 번에 쓰되 무료 한도를 못 넘기는 게 진짜 제약이었습니다. 기능이 아니라 비용이 병목이라, 토큰 버킷·캐시로 방어했습니다.',
    ...
    paar: {
      ...
      action: {
        ko: 'ODsay·Tmap·서울시 실시간 지하철 API를 백엔드에서 묶고, 지하철·버스·도보를 지도 위 하나의 폴리라인 경로로 그렸습니다. 토큰 버킷·24h 캐시로 호출을 방어했습니다.',
